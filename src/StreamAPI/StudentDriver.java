import java.util.Arrays;
import java.util.Comparator;
import java.util.List;
import java.util.Map;
import java.util.stream.Collectors;
import java.util.stream.Stream;

public class StudentDriver {

    public static void main(String[] args) {
        List<Student> studentList = Stream.of(
                        new Student(1, "Rohit", 30, "Male", "Mechanical Engineering", "Mumbai", 122, Arrays.asList("+912632632782", "+1673434729929")),
                        new Student(2, "Pulkit", 56, "Male", "Computer Engineering", "Delhi", 67, Arrays.asList("+912632632762", "+1673434723929")),
                        new Student(3, "Ankit", 25, "Female", "Mechanical Engineering", "Kerala", 164, Arrays.asList("+912632633882", "+1673434709929")),
                        new Student(4, "Satish Ray", 30, "Male", "Mechanical Engineering", "Kerala", 26, Arrays.asList("+9126325832782", "+1671434729929")),
                        new Student(5, "Roshan", 23, "Male", "Biotech Engineering", "Mumbai", 12, Arrays.asList("+012632632782")),
                        new Student(6, "Chetan", 24, "Male", "Mechanical Engineering", "Karnataka", 90, Arrays.asList("+9126254632782", "+16736784729929")),
                        new Student(7, "Arun", 26, "Male", "Electronics Engineering", "Karnataka", 324, Arrays.asList("+912632632782", "+1671234729929")),
                        new Student(8, "Nam", 31, "Male", "Computer Engineering", "Karnataka", 433, Arrays.asList("+9126326355782", "+1673434729929")),
                        new Student(9, "Sonu", 27, "Female", "Computer Engineering", "Karnataka", 7, Arrays.asList("+9126398932782", "+16563434729929", "+5673434729929")),
                        new Student(10, "Shubham", 26, "Male", "Instrumentation Engineering", "Mumbai", 98, Arrays.asList("+912632646482", "+16734323229929")))
                .collect(Collectors.toList());

        System.out.println("Printing origin student list");
        System.out.println(studentList.toString());

        System.out.println("Print students having rank 50-100");
        studentList.stream().filter( s -> {return s.getRank() > 50 && s.getRank() < 100 ;})
                .forEach(n -> System.out.println(n.toString()));

        System.out.println("Student stays in Karnataka & Sort by name");
        studentList.stream().filter( student -> student.getCity().equalsIgnoreCase("Karnataka"))
                .sorted((student1,student2) -> {
                    return  student1.getFirstName().compareTo(student2.getFirstName());
                }).forEach(n -> System.out.println(n.toString()));

        System.out.println("Extract all unique departments");
        studentList.stream().map(n -> n.getDept()).distinct().forEach(n -> System.out.println(n));

        System.out.println("Fetch all the contact numbers");

        //studentList.stream().map(n -> n.getContacts()).forEach(n -> n.forEach(s-> System.out.println(s)));
        studentList.stream().flatMap(student -> student.getContacts().stream()).distinct()
                .forEach(n -> System.out.println(n));

        System.out.println("Group students by department name");

        Map<String,List<Student>> map =  studentList.stream().collect(Collectors.groupingBy(Student::getDept));
        System.out.println(map);

        System.out.println("Group student count by department name");
        Map<String,Long> mapC =  studentList.stream().collect(Collectors.groupingBy(Student::getDept, Collectors.counting()));
        System.out.println(mapC);

        System.out.println("Find average age of male and female students");
        //Gropu by gender then use avg

        studentList.stream().collect(Collectors.groupingBy(Student::getGender,Collectors.averagingInt(Student::getAge)))
                .forEach((k,v) -> System.out.println(k +" "+v));

        System.out.println("Find the highest rank in each department");

        studentList.stream().collect(Collectors.groupingBy(Student::getDept, Collectors.minBy(Comparator.comparing(Student::getRank))))
                .forEach((k,v) -> System.out.println(k +" "+v));

        System.out.println("Find the student having second rank");
        Student secondRank = studentList.stream().sorted(Comparator.comparing(Student::getRank)).skip(1).findFirst().get();
        System.out.println("Second Rank student "+secondRank);








    }
}
